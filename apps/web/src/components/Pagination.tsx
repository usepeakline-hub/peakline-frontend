...
/**
 * "Showing 10 of 50 entries" + a page-size `Select` + numbered prev/next
 * controls — the exact pagination bar both `RecentPaymentsSection` (over a
 * fixed fake dataset) and `PaymentsList` (over whatever the filters leave)
 * need, extracted here rather than duplicated once a second real usage
 * showed up. `onPageSizeChange` is optional — a caller with a fixed page
 * size (or no size picker in its mock) just omits it and the Select never
 * renders.
 */
function Pagination({
	page,
